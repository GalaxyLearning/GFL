...
gfl_tempdir = PathUtils.join(os_tempdir, "gfl_p")
...
class GflConf(object, metaclass=GflConfMetadata):
    ...
    __home_dir = PathUtils.join(PathUtils.user_home_dir(), ".gfl_p")
    ...
    def load(cls) -> None:
        ...
        :return:
        """
        base_config_path = PathUtils.join(PathUtils.src_root_dir(), "resources", "gfl_config.yaml")
        with open(base_config_path) as f:
            cls.__readonly_props = yaml.load(f, Loader=yaml.SafeLoader)

        path = PathUtils.join(cls.home_dir, "gfl_config.yaml")
    ...
    def load_logging_config(cls) -> None:
        ...
        if cls.get_property("debug"):
            ...
            data["loggers"]["gfl_p"]["level"] = "DEBUG"
    ...
    def generate_config(cls, path: str = None) -> None:
        ...
        :param path: the config file path, if it's None, will be replaced by './gfl_config.yaml'.
        :return:
        """
        if path is None:
            path = "gfl_config.yaml"
        src_path = PathUtils.join(PathUtils.src_root_dir(), "resources", "gfl_config.yaml")
