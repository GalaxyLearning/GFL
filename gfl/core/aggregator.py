...
class Aggregator(object):
    ...
    def __init__(self, work_mode, job_path, base_model_path, concurrent_num=5):
        ...
        self.fed_step = {}
        self.fed_avg_aggregator = FedAvg()
    ...
    def _exec(self, aggreagtor, job_model_pars, base_model_path, job_id, fed_step):
        aggreagtor._exec(job_model_pars, base_model_path, job_id, fed_step)


class StandaloneAggregator(Aggregator):
    def __init__(self, job_path, base_model_path, concurrent_num=5):
        super(StandaloneAggregator, self).__init__(job_path, base_model_path, concurrent_num)
        ...
        for job in job_list:
            ...
            if job_fed_step != fed_step and job_model_pars is not None:
                ...
                if job.get_aggregate_strategy() == FederateStrategy.FED_AVG:
                    self._exec(self.fed_avg_aggregator, job_model_pars, self.base_model_path, job.get_job_id(),
                               fed_step)
...
class ClusterAggregator(Aggregator):
    def __init__(self, job_path, base_model_path, concurrent_num=5):
        super(ClusterAggregator, self).__init__(job_path, base_model_path, concurrent_num)
